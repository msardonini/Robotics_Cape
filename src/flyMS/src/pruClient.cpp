...
pruClient::pruClient() : u(NUM_CHANNELS)
{
    ...
    {
        std::cout<< "\n Error : Could not create socket: " << sterror(errno) << std::endl;
        return NULL;
    }
   ...
   // if(inet_pton(AF_INET, argv[1], &serv_addr.sin_addr)<=0)
    ...
    {
        std::cout<< "\n  inet_pton error occured: " << sterror(errno) << std::endl;
        return NULL;
    } 
    ...
    {
        std::cout<< "\n Error : Connect Failed: " << sterror(errno) << std::endl;
		return NULL;
    }
	
	this->pruSenderThread = std::thread(&pruClient::pruSender, this);

	if(this->pruSenderThread == NULL)
	{
